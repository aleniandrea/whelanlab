...
winpath='qualtrics_UG.csv'
macpath='qualtrics_UG.csv'
...
class DataReader:
    ...
    def read_data(self):
        data=pd.read_csv(winpath,sep=',',index_col='ID')
    ...
    def process_df(self,data):
        ...
        self.define_sss_subset(data)
        self.define_bis_subset(data)
        self.define_kirby(data)         
        return data
...
qualtrics_ug=reader.process_df(data)
qualtrics_ug.iloc[:,2:]=qualtrics_ug.iloc[:,2:].apply(pd.to_numeric)

qualtrics_ug.to_csv('qualtrics_pandas.csv')
